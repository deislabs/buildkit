...
func dispatchRun(d *dispatchState, c *instructions.RunCommand, proxy *llb.ProxyEnv, sources []*dispatchState, dopt dispatchOpt) error {
	...
	args, opt, customname, err := processCmdLine(d, &c.ShellDependantCmdLine, c.String(), false)
	if err != nil {
		return err
	}
...
func dispatchCmd(d *dispatchState, c *instructions.CmdCommand) error {
	args, _, _, err := processCmdLine(d, &c.ShellDependantCmdLine, c.String(), true)
	if err != nil {
		return err
	}
...
func processCmdLine(d *dispatchState, c *instructions.ShellDependantCmdLine, name string, createLayer bool) (args []string, opt []llb.RunOption, customname string, err error) {
	args = c.CmdLine
	customname = name
		...
		if heredoc := parser.MustParseHeredoc(args[0]); heredoc != nil {
			if d.image.OS != "windows" && strings.HasPrefix(c.Files[0].Data, "#!") {
				...
				if createLayer {
					...
					a := llb.Copy(st, f, destPath, opts...)
					d.state = d.state.File(a,
						llb.WithCustomName("[internal] preparing inline document"),
					)
					err = commitToHistory(&d.image, fmt.Sprintf("COPY <<%s %s", c.Files[0].Name, destPath), true, &d.state)
					if err != nil {
						return
					}
				} else {
